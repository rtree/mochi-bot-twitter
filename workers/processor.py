...
class Processor:
    ...
    async def summarize_results_async(self, snippets):
        ...
        p_src = (
            ...
            - 続くツィート(検索結果の中で最も重要な順番に10個)
            　　*記事要約*
            　　*記事URL*
        　　　  {self.config.TWITTER_DELIMITER}
            投稿先はX(Twitter)なので、Markdownは使わないでください"
            - 区切りは1記事ごと{self.config.TWITTER_DELIMITER}の区切り文字のみ。
            180文字ごとに区切ること。区切り文字は文字数に含めない。また、要約の冒頭に箇条書きなどの ・ は含めないでください"
             - 参考記事のURLは要約に含めない"
             - 要約が終わった後に{self.config.TWITTER_DELIMITER}で切ったのち、締めのコメントをする。締めのコメントは内容からいきなり書き始めてくださ。つまり、 締めのコメント などの見出しはつけないでください"
             - 要約の文体も{self.config.AINAME}になるように気をつけてください"
             - 最後に参考記事のURLを投稿する"
             - 参考記事の各URLの前に必ず{self.config.TWITTER_DELIMITER}と書き、次の行にリンクを記載"
             以下が要約対象の検索結果です:"
              {snippets}"

            あなたは著名テクノロジー企業の創業者です。あなたは以下の内容を要約してください。
